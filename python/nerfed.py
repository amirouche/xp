...
from aiohttp.web import Request, HTTPException, StreamResponse, Response, HTTPNotFound
            ...
            if match:
                ...
                if kind:
                    if name in conversion.keys():
                        # FIXME: this check on all routes before the server starts
                        msg = 'Invalid route, there is already a pattern named %s'
                        msg = msg % name
                        raise Exception(msg)
                    else:
    ...
    def match(self, sub, method, path, infos):
        ...
        if match:
            if self.method and match.endpos != match.end():
                ...
                convert = lambda item: (item[0], self.convert(item[1], moreinfos[item[0]]))
                try:
...
class App(Sub, ServerHttpProtocol):
    ...
    def handle_request(self, message, payload):
        ...
        # log access
        self.log_access(message, None, response_message, self._loop.time() - now)
